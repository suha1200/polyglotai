...
# Hygiene filters
# --------------------
EN_TITLE_BOILERPLATE = re.compile(
    r"(cover|about this text|acknowledg|table of contents|index|bibliograph|copyright|license)",
    re.I,
)
...
def is_boilerplate_title(section_title: str) -> bool:
    return bool(section_title and EN_TITLE_BOILERPLATE.search(section_title))
...
# --------------------
def main():
    p = argparse.ArgumentParser(description="Chunk English sections from merged corpus with hygiene filters.")
    ...
    # Hygiene knobs
    p.add_argument("--min_chars", type=int, default=50, help="Minimum chars per chunk")
    p.add_argument("--min_words", type=int, default=6, help="Minimum words per chunk")
    p.add_argument("--digits_punct_drop", type=float, default=0.60, help="Drop if non-letters ratio > this")
    ...
    print(f"[en] Done. Read rows: {count_rows}, wrote chunks: {kept}, dropped: {dropped}")
    print(f"[en] Drop breakdown: {stats_by_reason}")
